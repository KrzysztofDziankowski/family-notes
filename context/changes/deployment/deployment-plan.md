...
external uptime monitoring, alerting, a restore drill, a rollback rehearsal, and
reboot verification are operational follow-ups until separately recorded as
complete; the release readiness gate does not cover them.

Releases are gated on internal, database-backed readiness. The revised release
script requires the root-owned helper and the fixed-path library
`/usr/local/libexec/family-notes/release-gate.sh` to be installed as a matched
protocol-`1` pair through the provider console, following
`mikrus-runbook.md` ("Install or Update the Release Gate Pair"). Until that
installation and the gate acceptance runs are recorded, they remain pending.
...
- Create a non-login `familynotes` runtime user and a separate `deploy` user that owns release preparation. Install a root-owned `/usr/local/sbin/family-notes-deploy` helper and grant `deploy` passwordless sudo access to that helper only. The helper validates release IDs and exposes only checks, backup, activation, rollback, status, application-log, release-gate version (`gate-version`), internal readiness (`health`), and acceptance-reboot operations. It sources the root-owned readiness library from the fixed path `/usr/local/libexec/family-notes/release-gate.sh`, with no environment or argument override, and fails closed before any action when that library is missing or does not report protocol version `1`.
- Install or update the library and helper only as a matched pair through the provider console: copy both into root-owned temporary files in their destination directories with final ownership and permissions (`root:root`, library `0644`, helper `0755`), verify checksums and `/bin/sh -n`, and retain the installed artifacts as `.previous` copies. Promote the library first with a same-filesystem rename (the previous helper does not load it), run the staged helper's `gate-version` and require `1`, then promote the helper with another same-filesystem rename. Verify ownership, permissions, syntax, and `gate-version` before any release, and restore the retained artifacts if a final verification fails.
...
3. The script first requires the helper's `gate-version` action to print exactly `1` and stops without mutation otherwise. It then fetches the bare server repository, creates a detached timestamped worktree for that exact SHA under `/srv/family-notes/releases/`, and runs `uv sync --locked --no-dev`.
...
7. The script prints service status and runs the helper's `health` action as the release-completion gate. The gate probes `/healthz/` through the Gunicorn Unix socket immediately and then every two seconds under a strict 30-second deadline, capping each `curl` attempt and sleep to the remaining budget; an attempt passes only on a successful response whose body is exactly `{"status": "ok"}`. The script prints `Deployment completed` only after the gate passes. On exhaustion it exits nonzero, states that the activated release and pre-release backup remain in place, and prints the helper's `status`, `logs`, and `health` commands; it performs no rollback, symlink switch, migration reversal, or database restore.
8. Public HTTPS is not part of the automated gate. Manually verify the public health endpoint, inspect journald through the helper, and complete a browser smoke test.
...
- **Failed application start or exhausted readiness gate:** activation has already switched `current` and restarted the service, so the new release is live and the pre-release backup remains in `/var/backups/family-notes/`. The release script stops nonzero without declaring success and prints the diagnostic commands; it does not roll back. Run the helper's `status`, `logs`, and `health` actions, then inspect environment-file permissions, Gunicorn import errors, database connectivity, and nginx upstream errors. The operator decides between a corrective release and the manual application rollback.
- **Release gate protocol mismatch:** a missing library, an incompatible protocol, or a helper without `gate-version` stops the release before any fetch, worktree, backup, or activation. Reinstall the matched pair, or restore the retained artifacts, through the provider console.
- **Post-release regression:** the operator, never the release script, decides to atomically repoint `current` to the previous release and restart the service. If the release applied a backward-incompatible schema change, roll forward with a corrective release; database restore requires human approval and a confirmed recovery point.
...
- `manage.py check --deploy` reports no unresolved production-critical findings. The `SECURE_HSTS_INCLUDE_SUBDOMAINS` and `SECURE_HSTS_PRELOAD` warnings are expected and accepted: the shared `mikrus.dev` parent domain is not controlled by FamilyNotes, so both settings stay off while `SECURE_HSTS_SECONDS` still applies HSTS to the application host.
- The matched release-gate library and helper are installed through the provider console with verified `root:root` ownership, non-user-writable permissions, clean `/bin/sh -n` checks, and `gate-version` output `1`.
- An operator-approved release shows immediate readiness or retries followed by exact internal health success, and `Deployment completed` only after the gate passes.
- A controlled failed-probe rehearsal exits nonzero, keeps the attempted release active and its pre-release backup in place, prints the diagnostic commands, performs no rollback, and leaves public HTTPS verification manual.
...
- Pending operator authorization: installation of the matched release-gate pair, an approved release through the readiness gate, and the controlled failed-probe rehearsal.
